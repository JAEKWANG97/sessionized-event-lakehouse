...
object RunManifestWriter {
  ...
  private def renderJson(
    ...
    val fields = Seq(
      ...
      "lookback_input" -> config.lookbackInput.map(jsonString).getOrElse("null"),
      "input_paths" -> jsonArray(config.inputPaths),
      ...
      "repartition_by_dt" -> config.repartitionByDt.toString,
