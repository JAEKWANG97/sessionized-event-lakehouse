...
추가 기간 처리는 단순히 새 파일을 append하는 문제가 아닙니다. 이미 존재하는 `dt` partition을 다시 처리할
수도 있고, 기간 경계에서 이전 기간 이벤트와 이어지는 세션도 고려해야 합니다.

이 문서에서는 샘플 데이터로 다음 세 가지를 확인했습니다.

1. 새로운 기간을 추가 처리할 때 기존 `dt` partition이 유지되는가
2. 이미 존재하는 기간을 다시 처리할 때 중복 append가 아니라 대상 `dt` partition만 교체되는가
3. 기간 경계에서 이어지는 세션을 `--lookback-input`으로 끊기지 않게 계산할 수 있는가

## 전제

애플리케이션은 입력 경로, 출력 경로, 처리 기간, Hive database/table 이름을 실행 인자로 받습니다.
결과는 KST 기준 `dt` partition으로 저장합니다.

Hive sync를 사용하는 실행에서는 결과를 final path에 바로 쓰지 않습니다. 먼저 staging path에 쓰고,
staging 결과의 row 수와 `dt` partition 목록을 확인한 뒤 `_versions/{run_id}` 경로로 이동합니다. 이후 이번
실행에 포함된 `dt` partition의 Hive location을 새 version path로 전환합니다.
...
이 방식의 의도는 결과 저장 루트 전체를 매번 삭제하는 것이 아니라, 이번 실행 결과에 포함된 `dt` partition만
추가하거나 전환하는 것입니다.
...
기존 `2019-10-01`, `2019-10-02` partition은 유지되고, 새 `2019-10-03` partition만 추가되었습니다.
...
`2019-10-02` partition은 기존 3건에 새 2건이 append되어 5건이 된 것이 아니라, 재처리 결과 2건으로
교체되었습니다.

## 4차 실행: lookback input으로 기간 경계 세션 검증

이 검증은 최종 저장 대상 기간의 row가 이전 기간 이벤트와 같은 세션으로 이어질 수 있는지 확인합니다.
...
최종 저장된 row는 `dt=2019-11-01`의 1건뿐입니다. 하지만 `session_event_seq=2`이고
`session_start_at_kst`가 저장 범위 이전인 `2019-10-31 23:58:00`을 가리킵니다. 즉 lookback input의
이전 이벤트를 세션 계산에 사용했고, 최종 저장은 요청한 KST `dt` 범위만 수행했습니다.
...
파일 시스템에서는 run_id별 version path도 확인됩니다.
...
샘플 검증 결과, 현재 구현은 추가 기간 처리와 동일 기간 재처리 모두에서 의도한 방식으로 동작했습니다.

- 추가 기간 처리: 기존 `dt` partition 유지 + 새 `dt` partition 추가
- 동일 기간 재처리: 대상 `dt` partition의 Hive location을 새 version path로 전환, 중복 append 없음
