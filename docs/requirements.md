...
| KST 기준 daily partition 처리 | `Sessionization.scala`, KST 기준 날짜별 저장 구간인 `dt` partition |
| 동일 `user_id` 내에서 `event_time` 간격이 5분 이상이면 새 세션 ID 생성 | `Sessionization.scala`, 이전 이벤트와 현재 이벤트를 비교하는 window function |
...
| 배치 장애 시 복구를 위한 장치 구현 | `RunManifestWriter.scala`, 임시 저장 후 검증된 결과만 최종 반영하는 구조, `docs/run-manifest.md` |
