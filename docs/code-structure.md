...
```text
CSV 읽기 → 세션 계산 → 날짜 필터 → 임시 저장 경로에 Parquet 저장 → 검증 → run_id별 보관 경로로 이동 → Hive가 읽는 날짜별 위치 전환
```
...
`--lookback-input`이 있으면 `input + lookback-input`을 함께 읽고, 최종 저장은
...
Spark가 CSV 타입을 스스로 추측하게 두면 실행마다 타입이 흔들릴 수 있으므로, 명시적인 schema를 사용한다.
...
2. KST timestamp와 날짜별 저장 구간인 `dt` partition 생성
...
결과를 lake path에 저장하고 Hive external table을 동기화한다. 여기서 lake path는
분석용 파일을 모아두는 저장 경로를 뜻한다.
최종 table이 읽는 날짜 구간을 바로 덮어쓰지 않고, 먼저 임시 저장 경로에 쓴 뒤
row 수와 날짜 구간 목록을 확인한다. Hive sync를 사용하는 경우 검증된 결과를
`_versions/{run_id}` 보관 경로로 이동하고, 각 `dt` partition을 Hive가 읽는 위치를
새 보관 경로로 전환한다.
...
배치 실행 상태를 manifest JSON으로 기록한다. manifest는 실행 결과를 구조화해서
남기는 요약 파일이다. 로그만으로는 어떤 실행이 완료됐는지,
어떤 기간을 다시 실행해야 하는지 판단하기 어렵기 때문에 `RUNNING`, `SUCCESS`,
`FAILED` 상태와 처리 범위, row 수, 날짜 구간 목록을 구조화해서 남긴다.
...
UTC event_time에서 KST 기준 `dt` 날짜 구간 생성
...
보조 컬럼으로 남기고, 다음 분석이나 쿼리에서 쓰는 ID는 `user_id`와 세션 시작 시각
기반으로 만든다.
