...
이게 이 과제에서 말하는 “추가 기간 처리에 대응 가능”의 핵심이다.
...
이 방식은 과제 데이터 규모에서는 충분히 현실적이고, 결과도 이해하기 쉽다.
...
하지만 이 두 방식은 구현 복잡도가 올라가므로, 과제 1차 구현에서는 문서화된 확장안으로 남긴다.
